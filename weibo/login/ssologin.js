function SSOController() {
    ...
    this.updateCookie = function () {
        try {
            ...
            var url = makeURL(ssoUpdateCookieUrl, {entry: me.getEntry(), callback: me.name + ".updateCookieCallBack"});
            excuteScript(me.scriptId, url)
        } catch (e) {
        }
        return true
    ...
    var loginByXMLHttpRequest = function (username, password, savestate, qrcode_flag) {
        ...
        var url = makeURL(ssoLoginUrl, {client: me.getClientType(), _: (new Date()).getTime()});
        try {
    ...
    var loginByIframe = function (username, password, savestate, qrcode_flag) {
        ...
        var action = makeURL(ssoLoginUrl, objMerge({client: me.getClientType()}, me.loginExtraFlag));
